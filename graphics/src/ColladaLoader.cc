...
std::vector<unsigned int> parseUints(const char *_str, size_t _reserveCount)
{
  ...
  while (true)
  {
    ...
    if (*start == '-')
    {
      ignerr << "Negative index in <p> element; truncating after "
            << result.size() << " value(s).\n";
      break;
    }
    errno = 0;
    ...
    {
      ignerr << "Index out of range in <p> element; truncating after "
            << result.size() << " value(s).\n";
      break;
    }
    ...
    const std::string &_id, int _minStride, SourceAccessor &_out)
{
  ...
  if (!floatArrayXml || !floatArrayXml->GetText())
  {
    ...
    if (count)
    {
      ignerr << _sourceName << " source missing float_array element, "
        << "or count is invalid.\n";
    }
    else
    {
      ignlog << _sourceName << " source has a float_array with a count of "
  ...
  switch (floatArrayXml->QueryIntAttribute("count", &_out.totCount))
  {
    ...
    case tinyxml2::XML_NO_ATTRIBUTE:
      ignerr << "<float_array> has no count attribute in " << _semantic
            << " element with id[" << _id << "]\n";
      return false;
    default:
      ignerr << "Invalid count attribute in " << _semantic
  ...
  if (!techniqueXml)
  {
    ignerr << "Unable to find technique_common element for " << _semantic
          << "s with id[" << _id << "]\n";
    return false;
  }
  ...
  switch (accessorXml->QueryIntAttribute("stride", &_out.stride))
  {
    case tinyxml2::XML_SUCCESS:
      break;
    case tinyxml2::XML_NO_ATTRIBUTE:
      ignerr << "<accessor> has no stride attribute in " << _semantic
            << " element with id[" << _id << "]\n";
      return false;
    default:
      ignerr << "Invalid stride attribute in " << _semantic
...
Mesh *ColladaLoader::Load(const std::string &_filename)
{
      ...
      (std::string(version) != "1.4.0" && std::string(version) != "1.4.1"))
  {
    ...
    ignwarn << "Invalid collada file. Must be version 1.4.0 or 1.4.1. "
                    ...
                    DuplicateMapPtr &_duplicates)
{
  ...
  auto positions = _values && !_values->empty()
      ? std::make_shared<std::vector<math::Vector3d>>(*_values)
      : std::make_shared<std::vector<math::Vector3d>>();
    ...
    DuplicateMapPtr &_duplicates)
{
  ...
  auto texcoords = _values && !_values->empty()
      ? std::make_shared<std::vector<math::Vector2d>>(*_values)
      : std::make_shared<std::vector<math::Vector2d>>();
  auto duplicates = _duplicates && !_duplicates->empty()
      ? std::make_shared<std::unordered_map<unsigned int, unsigned int>>(
          *_duplicates)
      : std::make_shared<std::unordered_map<unsigned int, unsigned int>>();
  ...
  else if (typeElem->FirstChildElement("texture"))
  {
    ...
    if (imageXml && imageXml->FirstChildElement("init_from"))
    {
      ...
      if (imgFile)
        ...
        ignerr << "Empty <init_from> element for texture in material\n";
    }
  }
}
    ...
    Mesh *_mesh)
{
  ...
  while (polylistInputXml)
  {
    ...
    else if (semantic == "TEXCOORD")
    {
      ...
      if (!texcoords[set])
        texcoords[set] = std::make_shared<std::vector<math::Vector2d>>();
      if (!texDupMap[set])
        texDupMap[set] =
          std::make_shared<std::unordered_map<unsigned int, unsigned int>>();
  ...
  {
    ignerr << "Polylist is missing a <vcount> element\n";
    return;
  }
  ...
  {
    ignerr << "Polylist is missing a <p> element\n";
    return;
  }
  ...
  for (unsigned int l = 0; l < vcounts.size(); ++l)
  {
    ...
    if (polyStart + static_cast<std::size_t>(vcounts[l]) * inputSize >
        indices.size())
    {
      ignerr << "Polylist <p> element has fewer indices than its <vcount> "
